...
def parse_accept_language(header,supported_langauage):
    header=header.split(", ")
    #print("rewfrewf",header)
    result=[]
    wildcard=False
    initial_dict=defaultdict(list)
    for lang in supported_langauage:
        lang_initial=lang.split("-")[0]
        initial_dict[lang_initial].append(lang)
    for language in header:
        if language=="*":
            wildcard=True
        lang=language.split("-")
        #print(lang[0])
        if lang[0] in initial_dict:
            result.extend(initial_dict[lang[0]])
    #result=list(set(result))
    if wildcard:
        for language in supported_langauage:
...
print(parse_accept_language("en-US, fr-CA, fr-FR", ["fr-FR", "en-US"]))  # ["en-US", "fr-FR"]
print(parse_accept_language("fr-CA, fr-FR", ["en-US", "fr-FR"]))         # ["fr-FR"]
print(parse_accept_language("en-US", ["en-US", "fr-CA"]))                # ["en-US"]
print(parse_accept_language("en", ["en-US", "fr-CA", "fr-FR"]))          # ["en-US"]
print(parse_accept_language("fr", ["en-US", "fr-CA", "fr-FR"]))          # ["fr-CA", "fr-FR"]
print(parse_accept_language("fr-FR, fr", ["en-US", "fr-CA", "fr-FR"]))   # ["fr-FR", "fr-CA"]
print(parse_accept_language("en-US, *", ["en-US", "fr-CA", "fr-FR"]))    # ["en-US", "fr-CA", "fr-FR"]
print(parse_accept_language("fr-FR, fr, *", ["en-US", "fr-CA", "fr-FR"])) # ["fr-FR", "fr-CA", "en-US"]