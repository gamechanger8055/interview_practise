...
def constructItenary(tickets):
    graph=defaultdict(list)
    for dept,arrival in tickets:
        heapq.heappush(graph[dept],arrival)

    path=[]
    def dfs(source):
        while graph[source]:
            arrival=heapq.heappop(graph[source])
            dfs(arrival)
        path.append(source)
...
print(constructItenary(tickets))